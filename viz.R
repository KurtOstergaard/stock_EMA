...
fig1 <- plot_ly(x = ~colnames(res1), y = ~rownames(res1),  z = ~res1) |>
  add_surface(contours = list(
    z = list(
      ...
      project=list(z=TRUE)))) |>
  plotly::layout(title = paste0(LS, ' Growth Rates'),
         scene = list(
...
fig2 <- plot_ly(x = ~colnames(res2), y = ~rownames(res2),  z = ~res2) |>
  add_surface(contours = list(
    z = list(
      ...
      highlightcolor="#ff0000",
      project=list(z=TRUE)))) |>
  plotly::layout(title = paste0(LS, ' Bliss (ICAGR / DD)'),
         scene = list(
           xaxis = list(title = paste0(slow_type, " slow moving avg")),
           yaxis = list(title = paste0(fast_type, " fast moving avg")),
...
fig3 <- plot_ly(x = ~colnames(res3), y = ~rownames(res3),  z = ~res3) |>
  add_surface(contours = list(
    z = list(
      ...
      highlightcolor="#ff0000",
      project=list(z=TRUE)))) |>
  plotly::layout(title = paste0(LS, ' Lake Ratio'),
         scene = list(
           xaxis = list(title = paste0(slow_type, " slow moving avg")),
           yaxis = list(title = paste0(fast_type, " fast moving avg")),
...
fig4 <- plot_ly(x = ~colnames(res4), y = ~rownames(res4),  z = ~res4) |>
  add_surface(contours = list(
    z = list(
      ...
      highlightcolor="#ff0000",
      project=list(z=TRUE)))) |>
  plotly::layout(title = paste0(LS, ' Drawdown (1-DD)'),
         scene = list(
           xaxis = list(title = paste0(slow_type, " slow moving avg")),
           yaxis = list(title = paste0(fast_type, " fast moving avg")),
